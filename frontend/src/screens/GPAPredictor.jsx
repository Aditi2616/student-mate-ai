...
import { Plus, Trash2, Zap, Target, Trophy, Calculator, AlertCircle, CheckCircle, Settings } from 'lucide-react';

const GPAPredictor = () => {
  const [subjects, setSubjects] = useState([]);
  const [targetCpi, setTargetCpi] = useState(8.0);
  // ✅ NEW: Custom Attendance Criteria (Default 75 rakha hai)
  const [attCriteria, setAttCriteria] = useState(75); 
  ...
  const addSubject = async () => {
    if (!input.name || !input.mid) return alert("Subject Name aur Mid marks bharo!");
    try {
  ...
  // 🎯 DYNAMIC ATTENDANCE LOGIC (Based on User Input)
  const getAttStatus = (att, tot) => {
    const a = parseInt(att) || 0;
    const t = parseInt(tot) || 0;
    const target = parseFloat(attCriteria) || 75; 

    if (t === 0) return { p: 0, msg: "No Classes", color: "text-slate-400", bg: "bg-slate-50", safe: true };
    
    const perc = (a / t) * 100;
    const decimalTarget = target / 100;

    if (perc >= target) {
      // Safe bunks formula based on custom target
      const bks = Math.floor((a - (decimalTarget * t)) / decimalTarget);
      return { 
        p: perc.toFixed(1), 
        msg: bks > 0 ? `${bks} Bunks Safe! 😎` : "On Edge", 
        color: "text-green-600", 
        bg: "bg-green-50",
        safe: true 
      };
    } else {
      // Required classes formula based on custom target
      const req = Math.ceil((decimalTarget * t - a) / (1 - decimalTarget));
      return { 
        p: perc.toFixed(1), 
        msg: `Attend ${req} more! ⚠️`, 
        color: "text-red-600", 
        bg: "bg-red-50",
        safe: false 
      };
    ...
    <div className="flex min-h-screen bg-slate-50 font-sans">
      ...
      <div className="flex-1 p-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-indigo-600 p-6 rounded-[2.5rem] text-white shadow-2xl col-span-1 md:col-span-2 flex items-center justify-between">
          ...
          <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 flex items-center justify-between shadow-sm">
            <div><p className="text-[10px] font-black text-slate-400 uppercase">GPA Target</p>
            <input type="number" className="text-3xl font-black text-indigo-600 outline-none w-20 bg-transparent" value={targetCpi} onChange={(e) => setTargetCpi(e.target.value)} /></div>
            <Target size={30} className="text-slate-200" />
          </div>

          {/* NEW: Attendance Criteria Input */}
          <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 flex items-center justify-between shadow-sm">
            <div><p className="text-[10px] font-black text-slate-400 uppercase">Criteria %</p>
            <input type="number" className="text-3xl font-black text-orange-500 outline-none w-20 bg-transparent" value={attCriteria} onChange={(e) => setAttCriteria(e.target.value)} /></div>
            <Settings size={30} className="text-slate-200" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-50 h-fit sticky top-8">
            ...
            <div className="space-y-5">
              ...
              <button onClick={addSubject} className="w-full bg-slate-900 text-white py-5 rounded-[1.5rem] font-black hover:bg-indigo-600 transition-all shadow-lg">Add Subject</button>
            </div>
          </div>
          ...
          <div className="xl:col-span-2 space-y-5">
            ...
            {subjects.map(sub => {
              ...
              return (
                <div key={sub._id} className="p-8 bg-white rounded-[3.5rem] border border-slate-100 shadow-sm flex flex-wrap items-center justify-between gap-6 hover:shadow-xl transition-all">
                    ...
                    <div className="mt-8 flex gap-16">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3">
                          <input type="number" className="w-24 p-4 bg-indigo-50 border-2 border-indigo-100 rounded-2xl text-center font-black text-indigo-600 outline-none text-xl" value={sub.end} onChange={(e) => setSubjects(subjects.map(s => s._id === sub._id ? {...s, end: e.target.value} : s))} />
                          <span className="text-slate-300 font-bold text-lg">/ {sub.endTotal}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  <button onClick={() => deleteSubject(sub._id)} className="p-5 text-slate-100 hover:text-red-500 rounded-full">
                    <Trash2 size={28}/>
                  </button>
                </div>
              );
